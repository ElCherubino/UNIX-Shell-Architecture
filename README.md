...
3. **AST Executor:** Traverses the tree node by node to execute the commands using low-level UNIX system calls (`fork`, `execve`, `pipe`, `dup2`, `waitpid`).

## Key Technical Achievements
* **Advanced Process Synchronization:** Handled complex piping (`|`) and file redirections (`>`, `<`, `>>`) without deadlocks or file descriptor leaks.
...
* **Quality Assurance:** Designed a comprehensive test suite covering standard use cases and obscure edge-cases (e.g., empty pipes, trailing spaces, complex quoting).
---
